#===============================================================================
#         FILE: frombam.eCLIP.pip.py  step3strand.py  
#
...
#				skiping 1st (download) and 2nd (mapping) steps
# 				step3: determine the strandness of eCLIP sample reads
#
...
def strandinfo_PAIR(logfile, outfile):
	f=open( logfile ).readlines()
	STRINFO = ''
	F = 1
	R = 1
...
########extract r2 reads from combined bam file 
	
	# cmd01="samtools view -hb -f 64 "+samoutdir+'/'+sample+".bam > "+samoutdir+'/'+sample+".r1.bam"
	cmd01="samtools view -hb -f 128 "+samoutdir+'/'+sample+".bam > "+samoutdir+'/'+sample+".r2.bam"
	print cmd01
	os.system(cmd01)

	bamfile=samoutdir+'/'+sample+'.r2.bam'
	...
	if seqtype=='SINGLE': 
		...
		STRINFO=strandinfo_PAIR(logfile,outfile)

	else:
		print " Library type ERROR, check your sample table!!! "
	
	cmd02="bamToBed -i "+samoutdir+'/'+sample+".r2.bam > "+samoutdir+'/'+sample+".bed"    ##this step finish in bash
	print cmd02
	os.system(cmd02)
